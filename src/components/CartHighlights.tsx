...
const CartBadgeSlider: FC = () => {
  const [currentIndex, setCurrentIndex] = useState<number>(0);
  const [isTransitioning, setIsTransitioning] = useState<boolean>(true);

  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  const extendedData = [...CART_HIGHLIGHTS_DATA, CART_HIGHLIGHTS_DATA[0]];
  ...
  useEffect(() => {
    if (currentIndex === CART_HIGHLIGHTS_DATA.length) {
...
export default CartBadgeSlider;
